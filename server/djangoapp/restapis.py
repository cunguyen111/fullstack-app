...
backend_url = _normalize_base(
    os.getenv("backend_url", "http://localhost:3030"))
sentiment_analyzer_url = _normalize_base(
...
def get_request(endpoint: str, **kwargs):
    ...
    try:
        ...
        resp = requests.get(
            request_url, params=kwargs or None, timeout=DEFAULT_TIMEOUT)
...
def analyze_review_sentiments(text: str):
    ...
    path_style_url = urljoin(
        sentiment_analyzer_url, "analyze/" + quote_plus(str(text)))
    query_style_url = urljoin(sentiment_analyzer_url, "analyze")

    # Thử path-style trước, fallback sang query-style
    for request_url, params in ((path_style_url, None),
                                (query_style_url, {"text": text})):
        try:
            print(f"GET {request_url} params={params}")
            resp = requests.get(
                request_url, params=params, timeout=DEFAULT_TIMEOUT)
            resp.raise_for_status()
            data = resp.json()
            # Chuẩn hóa output
...
def post_review(data_dict: dict):
    ...
    try:
        ...
        resp = requests.post(
            request_url, json=data_dict, timeout=DEFAULT_TIMEOUT)
        resp.raise_for_status()
        data = resp.json()
        print(data)
        return data
